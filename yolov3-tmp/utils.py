...
def data_generate_train(batch_size=64):
    ...
    while True:
        ...
        y_true = [np.zeros(shape=(batch_size, shape[0], shape[1], len(anchors) // 3, 5 + num_classes)) for shape in
             grid_shape]
        begin = time.time()
        for i in range(batch_size):
            ...
            for y_true_box in y_true_boxes:
                ...
                grid_shape_h_select = int(y / img_h * grid_shape_h)
                grid_shape_w_select = int(x / img_w * grid_shape_w)

                tx = x / img_w * grid_shape_w - grid_shape_w_select
                ty = y / img_h * grid_shape_h - grid_shape_h_select
                tw = np.log(w / anchors[iou_max_index][0])
                th = np.log(w / anchors[iou_max_index][1])

                y_true[grid_shape_index][i, grid_shape_h_select, grid_shape_w_select, anchor_index, :4] = np.array([tx, ty, tw, th])
                y_true[grid_shape_index][i, grid_shape_h_select, grid_shape_w_select, anchor_index, 4] = 1
                y_true[grid_shape_index][i, grid_shape_h_select, grid_shape_w_select, anchor_index, 5 + class_index] = 1
            count = count + 1
            if count == lines_len:
                shuffle(lines)
                count = 0
...
if __name__ == '__main__':
    ...
    data_gen = data_generate_train(batch_size=16)
