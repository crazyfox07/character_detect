...
def preprocess_true_boxes(true_boxes):
    ...
    y_true = np.zeros(shape=(b_s, grid_shape[0], grid_shape[1], 5 + num_classes), dtype='float32')
    for i in range(b_s):
        for j in range(true_boxes.shape[1]):
            ...
            if true_boxes[i, j, 2] > 0:
                ...
                y_true[i, grid_y, grid_x, 2:4] = true_boxes[i, j, 2:4] / (input_shape[::-1])
                y_true[i, grid_y, grid_x, 0] = true_boxes[i, j, 0] / grid_cell_shape[1] - grid_x
                y_true[i, grid_y, grid_x, 1] = true_boxes[i, j, 1] / grid_cell_shape[0] - grid_y
                y_true[i, grid_y, grid_x, 4] = 1  # 置信度，判断是否有box
                y_true[i, grid_y, grid_x, 5 + c] = 1
            else:
                continue
